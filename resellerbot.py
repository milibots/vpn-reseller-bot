    ...
    async def __call__(self, handler, event, data):
        ...
        if config.get("MAINTENANCE_MODE") and not is_admin:
            ...
            if isinstance(event, Message):
                await event.reply(maint_txt, parse_mode=None)
        ...
        with SessionLocal() as db:
            ...
            if db_user and db_user.is_banned:
                ...
                if isinstance(event, Message):
                    await event.reply(ban_txt, parse_mode=None)
                elif isinstance(event, CallbackQuery):
        ...
        if chat_id:
            try:
                ...
                if isinstance(event, Message):
                    await event.reply(msg_text, reply_markup=kb, parse_mode=None)
                elif isinstance(event, CallbackQuery):
                    await event.message.edit_text(msg_text, reply_markup=kb, parse_mode=None)
                    await event.answer()
                return
        return await handler(event, data)
...
async def start_handler(message: Message, state: FSMContext):
    ...
    with SessionLocal() as db:
        ...
        await safe_reply(message, welcome_txt, reply_markup=main_menu_keyboard(message.from_user.id))
...
async def main_menu_callback(callback: CallbackQuery, state: FSMContext):
    ...
    with SessionLocal() as db:
        ...
        await safe_edit(callback.message, welcome_txt, reply_markup=main_menu_keyboard(callback.from_user.id))
...
async def support_callback(callback: CallbackQuery):
    ...
    await safe_edit(callback.message, support_txt, reply_markup=kb)
...
async def wallet_callback(callback: CallbackQuery):
    with SessionLocal() as db:
        ...
        await safe_edit(callback.message, txt, reply_markup=kb)
        await callback.answer()
...
async def process_charge_amount(message: Message, state: FSMContext):
    ...
    await safe_reply(message, payment_txt, reply_markup=kb)
...
async def process_receipt_photo(message: Message, state: FSMContext):
    ...
    await message.reply("✅ رسید شما با موفقیت ثبت شد و در انتظار تایید مدیریت قرار گرفت. به محض بررسی وضعیت آن به شما اطلاع داده خواهد شد.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
    await state.clear()
...
async def process_crypto_asset(callback: CallbackQuery, state: FSMContext):
    ...
    await safe_edit(callback.message, crypto_instructions, reply_markup=kb)
...
async def process_crypto_receipt(message: Message, state: FSMContext):
    ...
    if not photo_id and not tx_hash:
        return await message.reply("❌ خطا: لطفاً یک فیش تصویری ارسال کنید یا کد پیگیری (TXID) تراکنش را به صورت متنی وارد کنید.", parse_mode=None)
    ...
    await message.reply("✅ رسید تراکنش رمزارز شما با موفقیت ثبت شد و جهت بررسی در اختیار مدیریت قرار گرفت.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
    await state.clear()
    ...
    for admin_id in config["ADMIN_IDS"]:
        try:
            if photo_id:
                await bot.send_photo(chat_id=admin_id, photo=photo_id, caption=admin_cap, reply_markup=admin_markup, parse_mode=None)
            else:
                await bot.send_message(chat_id=admin_id, text=admin_cap, reply_markup=admin_markup, parse_mode=None)
        except Exception as e:
...
async def buy_service_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if config.get("SHOP_CLOSED") and callback.from_user.id not in config["ADMIN_IDS"]:
        ...
        return await safe_edit(callback.message, closed_txt, reply_markup=back_to_menu_keyboard())
        
    await callback.message.edit_text("⏳ در حال دریافت لیست پلن‌ها از سرور...", parse_mode=None)
    
    plans_data = api.get_plans()
    if not plans_data or not plans_data.get("success"):
        return await callback.message.edit_text("❌ خطا در ارتباط با سرور یا دریافت اطلاعات پلن‌ها. لطفا مجدداً تلاش کنید.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
        
    plans = plans_data.get("plans", [])
    if not plans:
        return await callback.message.edit_text("🛒 در حال حاضر پلن فعالی جهت فروش موجود نیست.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
...
async def buy_plan_callback(callback: CallbackQuery, state: FSMContext):
    ...
    with SessionLocal() as db:
        ...
        if user.balance < price:
            ...
            return await safe_edit(callback.message, insufficient_txt, reply_markup=kb)
    ...
    await callback.message.edit_text(name_instructions, reply_markup=kb, parse_mode=None)
...
async def process_service_name(message: Message, state: FSMContext):
    ...
    if not re.match(r"^[a-zA-Z0-9]{3,12}$", name):
        return await message.reply("❌ خطا: نام سرویس فقط باید شامل حروف انگلیسی و اعداد بین ۳ تا ۱۲ کاراکتر باشد. مجدداً ارسال کنید:", parse_mode=None)
...
async def render_checkout_invoice(message_to_send: Message, state: FSMContext):
    ...
    if isinstance(message_to_send, Message):
        await safe_reply(message_to_send, confirm_txt, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
    else:
        await safe_edit(message_to_send, confirm_txt, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def process_discount_code_input_checkout(message: Message, state: FSMContext):
    ...
    with SessionLocal() as db:
        ...
        if not dc:
            return await message.reply("❌ خطا: این کد تخفیف وجود ندارد یا غیرفعال شده است. مجددا تلاش کنید یا انصراف دهید:", parse_mode=None)
            
        if dc.expiry_date and dc.expiry_date < datetime.utcnow():
            return await message.reply("❌ خطا: مدت اعتبار این کد تخفیف به پایان رسیده است.", parse_mode=None)
            
        if dc.used_count >= dc.usage_limit:
            return await message.reply("❌ خطا: سقف تعداد دفعات استفاده از این کد تخفیف پر شده است.", parse_mode=None)
            
        if dc.specific_user_id and dc.specific_user_id != message.from_user.id:
            return await message.reply("❌ خطا: این کد تخفیف مخصوص یک کاربر دیگر طراحی شده است و شما مجاز به استفاده از آن نیستید.", parse_mode=None)
            
    await state.update_data(applied_discount_code=dc.code)
    await state.set_state(Form.waiting_for_service_name)
    
    await message.reply(f"✅ کد تخفیف {dc.code} با موفقیت بر روی فاکتور شما اعمال گردید.", parse_mode=None)
    await render_checkout_invoice(message, state)
...
async def confirm_buy_final_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if not plan_id or not name:
        return await callback.message.edit_text("❌ خطا در بازیابی اطلاعات نشست خرید. مجدداً اقدام کنید.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
        
    await callback.message.edit_text("⏳ در حال برقراری ارتباط با سرور و پیکربندی اکانت...", parse_mode=None)
    
    with SessionLocal() as db:
        user = db.query(DBUser).filter(DBUser.telegram_id == callback.from_user.id).first()
        if not user or user.balance < price:
            return await callback.message.edit_text("❌ موجودی حساب شما کافی نیست یا کاربر یافت نشد.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
        ...
        if status_code == 200 and res_data.get("success"):
            ...
            await safe_edit(callback.message, success_txt, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb_list))
            ...
            for admin_id in config["ADMIN_IDS"]:
                try:
                    await bot.send_message(chat_id=admin_id, text=admin_msg, parse_mode=None)
                except Exception as e:
...
async def my_services_callback(callback: CallbackQuery):
    await callback.message.edit_text("⏳ در حال دریافت لیست سرویس‌های شما از سرور...", parse_mode=None)
    ...
    if not services:
        return await callback.message.edit_text("🌐 شما هیچ سرویس فعالی ثبت نکرده‌اید.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
...
async def srv_view_callback(callback: CallbackQuery):
    ...
    with SessionLocal() as db:
        ...
        if not srv:
            return await callback.message.edit_text("❌ سرویس مورد نظر در سیستم یافت نشد.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
            
    await callback.message.edit_text("⏳ در حال دریافت جزئیات و ترافیک لحظه‌ای از سرور...", parse_mode=None)
    ...
    if not details or not details.get("success"):
        ...
        return await callback.message.edit_text(detail_txt, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb), parse_mode=None)
    ...
    await safe_edit(callback.message, detail_txt, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
    await callback.answer()
...
async def srv_toggle_callback(callback: CallbackQuery):
    ...
    await callback.message.edit_text("⏳ در حال ارسال دستور تغییر وضعیت به سرور...", parse_mode=None)
...
async def srv_qr_callback(callback: CallbackQuery):
    ...
    await callback.message.edit_text("⏳ در حال تولید کد QR اختصاصی...", parse_mode=None)
    try:
...
async def srv_renew_callback(callback: CallbackQuery):
    ...
    srv_id = int(callback.data.split("_")[2])
    await callback.message.edit_text("⏳ در حال استعلام مشخصات پلن فعلی از سرور...", parse_mode=None)
    
    with SessionLocal() as db:
        srv = db.query(DBService).get(srv_id)
        if not srv or not srv.plan_id:
            return await callback.message.edit_text("❌ خطا: کد پلن اصلی متصل به این سرویس یافت نشد.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
            
        target_plan_id = srv.plan_id

    plans_data = api.get_plans()
    if not plans_data or not plans_data.get("success"):
        return await callback.message.edit_text("❌ خطا در دریافت پلن‌ها از سرور.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
    ...
    if not matched_plan:
        return await callback.message.edit_text("❌ خطا: این پلن دیگر روی سرور اصلی فعال نیست یا امکان تمدید مستقیم آن وجود ندارد.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
...
async def renew_confirm_callback(callback: CallbackQuery):
    ...
    await callback.message.edit_text("⏳ در حال تمدید اشتراک بر روی سرور...", parse_mode=None)
...
async def srv_extend_gb_init_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("⏳ در حال استعلام قیمت ترافیک اضافه از سرور...", parse_mode=None)
    ...
    plans_data = api.get_plans()
    if not plans_data or not plans_data.get("success"):
         return await callback.message.edit_text("❌ خطا در استعلام مشخصات فنی پلن‌ها.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
    ...
    prompt_txt = (
        f"⚡️ خرید حجم اضافه برای سرویس {srv.name}\n\n"
        f"💵 قیمت هر گیگابایت ترافیک اضافه: {int(gb_price):,} تومان\n\n"
...
async def process_extend_gb_amount(message: Message, state: FSMContext):
    ...
    invoice_txt = (
        ...
        f"⚡️ مقدار حجم اضافه: {gb_amount:.2f} GB\n"
        f"💵 قیمت به ازای هر گیگابایت: {int(gb_price):,} تومان\n\n"
        f"💳 مجموع هزینه قابل پرداخت: {cost:,} تومان\n"
        f"👛 موجودی کیف پول فعلی شما: {int(user.balance):,} تومان"
    )
    ...
    await safe_reply(message, invoice_txt, reply_markup=kb)
...
async def confirm_extend_gb_final_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if not srv_id or not gb_amount:
         return await callback.message.edit_text("❌ خطا: اطلاعات نشست خرید منقضی شده است.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
    ...
    await callback.message.edit_text("⏳ در حال اعمال حجم ترافیک اضافه بر روی سرور...", parse_mode=None)
    ...
    if status_code == 200 and res_data.get("success"):
        ...
        admin_msg = (
            ...
            f"👤 کاربر: {db_user.first_name or 'نامشخص'} ({db_user.telegram_id})\n"
            f"🖥 سرویس: {srv.name}\n"
            f"📊 حجم افزوده شده: {gb_amount:.2f} GB\n"
            f"💵 هزینه کسر شده: {cost:,} تومان"
        )
        for admin_id in config["ADMIN_IDS"]:
            try:
                await bot.send_message(chat_id=admin_id, text=admin_msg, parse_mode=None)
...
async def admin_panel_callback(callback: CallbackQuery, state: FSMContext):
    ...
    kb.append([InlineKeyboardButton(text="🔙 بازگشت به منوی اصلی", callback_data="btn_main_menu")])
    await callback.message.edit_text(txt, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb), parse_mode=None)
    await callback.answer()
...
async def adm_sub_branding_menu_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await safe_edit(callback.message, txt, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
    await callback.answer()
...
async def brand_set_key_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if key == "logo_shape_menu":
        ...
        return await callback.message.edit_text("🖼 شکل قاب لوگوی اختصاصی برند خود را انتخاب کنید:", reply_markup=kb, parse_mode=None)
        ...
        return await callback.message.edit_text("🔋 قالب نمایش مصرف ترافیک در صفحه اشتراک را مشخص کنید:", reply_markup=kb, parse_mode=None)
...
async def brand_save_menu_options(callback: CallbackQuery):
    ...
    await callback.message.edit_text("⏳ در حال ثبت و اعمال تنظیمات قالب روی سرور اختصاصی...", parse_mode=None)
    ...
    if res and res.get("success"):
        ...
        await callback.answer("❌ خطا در تغییر تنظیمات روی سرور اصلی.", show_alert=True)
...
async def brand_toggle_blur_callback(callback: CallbackQuery):
    ...
    await callback.message.edit_text("⏳ در حال دریافت تنظیمات فعلی پس‌زمینه از سرور...", parse_mode=None)
    ...
    await callback.message.edit_text("✨ تنظیم افکت تار شدن پس‌زمینه (Blur) صفحه اشتراک:", reply_markup=kb, parse_mode=None)
    await callback.answer()
...
async def brand_reset_defaults_confirmed_callback(callback: CallbackQuery):
    ...
    await callback.message.edit_text("⏳ در حال بازگردانی تنظیمات قالب به مقادیر پایه سیستمی...", parse_mode=None)
    res = api.update_brand(payload)
    if res and res.get("success"):
...
async def save_brand_field_api(message: Message, state: FSMContext, field_key: str, value):
    ...
    await message.reply("⏳ در حال همگام‌سازی و ذخیره‌سازی داده‌های قالب روی سرور اصلی...", parse_mode=None)
    ...
    if res and res.get("success"):
        await message.reply(
            f"✅ ویژگی برند {field_key} با موفقیت روی سرور اعمال و ذخیره شد.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def process_gift_all_users(message: Message, state: FSMContext):
    ...
    if total_users == 0:
        return await message.reply("❌ هیچ کاربر فعالی در سیستم جهت دریافت هدیه یافت نشد.", parse_mode=None)
        
    progress_message = await message.reply("⏳ در حال آغاز فرآیند تخصیص اعتبار هدیه به کل کاربران...", parse_mode=None)
    
    success_count = 0
    failed_count = 0
    checkpoint = max(1, int(total_users / 10))
...
async def process_dc_code_step(message: Message, state: FSMContext):
    ...
    if not re.match(r"^[A-Z0-9]{3,15}$", code_str):
        return await message.reply("❌ خطا: کد تخفیف فقط باید شامل حروف انگلیسی و اعداد بین ۳ تا ۱۵ کاراکتر باشد. مجدداً وارد کنید:", parse_mode=None)
        
    with SessionLocal() as db:
        existing = db.query(DBDiscountCode).filter(DBDiscountCode.code == code_str).first()
        if existing:
            return await message.reply("❌ خطا: این کد تخفیف قبلاً در دیتابیس تعریف شده است. لطفا یک کد دیگر وارد کنید:", parse_mode=None)
    ...
    await message.reply("🎟 مرحله ۲: نوع اعمال تخفیف را مشخص کنید:", reply_markup=kb, parse_mode=None)
...
async def process_dc_type_step(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(prompt, parse_mode=None)
...
async def process_dc_value_step(message: Message, state: FSMContext):
    ...
    if not val_str.isdigit():
        return await message.reply("❌ خطا: لطفاً مقدار را به صورت یک عدد عددی انگلیسی وارد کنید:", parse_mode=None)
...
async def process_dc_limit_step(message: Message, state: FSMContext):
    ...
    if not limit_str.isdigit():
        return await message.reply("❌ خطا: لطفاً تعداد را به صورت عدد انگلیسی وارد کنید:", parse_mode=None)
        
    limit = int(limit_str)
    if limit < 1:
        return await message.reply("❌ خطا: حداقل تعداد دفعات استفاده باید ۱ باشد. مجدداً وارد کنید:", parse_mode=None)
        
    await state.update_data(new_dc_limit=limit)
    await message.reply("🎟 مرحله ۵: طول عمر و مدت اعتبار کد تخفیف را به روز وارد نمایید (مثلا عدد 7 برای اعتبار یک هفته‌ای. جهت بی‌محدودیت بودن عدد 0 بفرستید):", parse_mode=None)
...
async def process_dc_expiry_step(message: Message, state: FSMContext):
    ...
    if not days_str.isdigit():
        return await message.reply("❌ خطا: طول عمر اعتبار را به صورت عدد عددی انگلیسی وارد کنید:", parse_mode=None)
    ...
    await message.reply(
        ...
        "(جهت ساخت کد عمومی عدد 0 را ارسال کنید و در غیر این صورت آیدی عددی تلگرام کاربر هدف را بفرستید)",
        parse_mode=None
    )
...
async def process_dc_final_save_step(message: Message, state: FSMContext):
    ...
    if not user_restrict_str.isdigit():
        return await message.reply("❌ خطا: لطفاً آیدی عددی معتبر یا عدد ۰ بفرستید:", parse_mode=None)
...
async def process_config_json_upload_save(message: Message, state: FSMContext):
    ...
    if not doc.file_name.endswith(".json"):
        return await message.reply("❌ خطا: فایل ارسالی باید دارای پسوند معتبر .json باشد.", parse_mode=None)
        
    await message.reply("⏳ در حال دریافت و اعتبارسنجی ساختار فایل پیکربندی...", parse_mode=None)
...
async def process_cfg_key_value_save(message: Message, state: FSMContext):
    ...
    if key == "FORCE_JOIN_CHAT_ID":
        try:
            ...
            return await message.reply("❌ خطا: آیدی کانال حتما باید به صورت یک عدد علامت‌دار (مانند -100123456789) وارد شود.", parse_mode=None)
    else:
        config[key] = val_str
...
async def adm_txt_edit_key(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "(دقت کنید متغیرهایی مانند {balance}، {amount} و... در صورت وجود دست-نخورده باقی بمانند)",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def process_texts_json_upload_save(message: Message, state: FSMContext):
    ...
    await message.reply("⏳ در حال دریافت و اعتبارسنجی فایل متون...", parse_mode=None)
...
async def adm_stats_callback(callback: CallbackQuery):
    ...
    await callback.message.edit_text("⏳ در حال استخراج و تحلیل داده‌های مالی...", parse_mode=None)
    ...
    await callback.message.edit_text(txt, reply_markup=kb, parse_mode=None)
    await callback.answer()
...
async def adm_central_bal_callback(callback: CallbackQuery):
    ...
    await callback.message.edit_text("⏳ در حال استعلام تراز مالی وب‌سرویس...", parse_mode=None)
    res = api.get_balance()
    if res and res.get("success"):
...
async def adm_view_users_callback(callback: CallbackQuery):
    ...
    for u in users:
        ...
        txt += f"👤 {status_symbol} {u.first_name or 'Unknown'} ({u.telegram_id}){ban_text}\n"
        txt += f"▫️ رتبه: {rank}\n"
...
async def admin_review_pending_callback(callback: CallbackQuery):
    ...
    with SessionLocal() as db:
        ...
        if not tx:
            return await callback.message.edit_text("✅ تمام فیش‌های واریزی بررسی شده‌اند و در حال حاضر فیش جدیدی موجود نیست.", reply_markup=back_to_menu_keyboard(), parse_mode=None)
...
async def admin_decision_callback(callback: CallbackQuery):
    ...
    with SessionLocal() as db:
        ...
        if action == "approve":
            ...
            try:
                await callback.message.edit_caption(caption=f"✅ رسید به مبلغ {int(tx.amount):,} تومان توسط ادمین تایید شد.", parse_mode=None)
            except Exception:
                await callback.message.edit_text(text=f"✅ رسید به مبلغ {int(tx.amount):,} تومان توسط ادمین تایید شد.", parse_mode=None)

            for admin_id in config["ADMIN_IDS"]:
                if admin_id != callback.from_user.id:
                    try:
                        await bot.send_message(chat_id=admin_id, text=f"💰 افزایش تراز تایید شده: کاربر {user.telegram_id} مبلغ {tx.amount:,} تومان دریافت کرد.", parse_mode=None)
                    except Exception: pass
        else:
            tx.status = "rejected"
            db.commit()
            ...
            try:
                await callback.message.edit_caption(caption=f"❌ رسید به مبلغ {int(tx.amount):,} تومان توسط ادمین رد شد.", parse_mode=None)
            except Exception:
                await callback.message.edit_text(text=f"❌ رسید به مبلغ {int(tx.amount):,} تومان توسط ادمین رد شد.", parse_mode=None)
                ...
                for s in db.query(DBService).all():
                    ...
                    if alerts:
                        ...
                        for alert_type, title in alerts:
                            ...
                            try:
                                await bot.send_message(chat_id=user.telegram_id, text=msg_usr, reply_markup=kb_usr, parse_mode=None)
                            except Exception:
                                pass
                            msg_adm = (
                            ...
                            for aid in config["ADMIN_IDS"]:
                                try:
                                    await bot.send_message(chat_id=aid, text=msg_adm, reply_markup=kb_adm, parse_mode=None)
                                except Exception:
                                    pass
...
async def adm_usr_view_callback(callback: CallbackQuery):
    ...
    with SessionLocal() as db:
        ...
        txt = (
            ...
            f"نام کاربری: @{u.username or ''}\n"
            f"تراز حساب: {int(u.balance):,} تومان\n"
            f"رتبه: {rank}"
        )
        kb = InlineKeyboardMarkup(inline_keyboard=[
